...
final class InertiaTimerTask implements Runnable {
    ...
    public final void run() {
        ...
        if (Math.abs(a) >= 0.0F && Math.abs(a) <= 20F) {
            wheelView.handler.sendEmptyMessageDelayed(MessageHandler.WHAT_SMOOTH_SCROLL_INERTIA,60);
        ...
        if (!wheelView.isLoop) {
            float itemHeight = wheelView.lineSpacingMultiplier * wheelView.itemTextHeight;
